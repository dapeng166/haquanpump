  ...
  return (
    <article className="group glass-card flex h-full flex-col overflow-hidden p-0">
      ...
      >
        ...
        <span className="absolute left-4 top-4 rounded-full border border-white/20 bg-charcoal-950/70 px-3 py-1 text-xs font-medium text-navy-100 backdrop-blur">
          {product.seriesName}
        </span>
      </Link>
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <div className="mt-auto flex items-center justify-between gap-3 pt-5">
          ...
          <a
            ...
            className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-white/15 bg-white/[0.03] text-navy-100/70 transition-colors hover:border-accent/50 hover:text-accent-300"
          >
...
function SpecMini({
  ...
  return (
    <div>
      <div className="flex items-center justify-center gap-1 text-accent-300/80">{icon}</div>
      <div className="mt-1 truncate text-xs font-semibold text-white" title={`${value} ${unit}`}>
        {value}
      </div>
      <div className="text-[0.65rem] uppercase tracking-wide text-navy-100/40">
        {label} ({unit})
      </div>
    </div>
  );
